...
```bash
npx skills add https://github.com/drgarbage/ag-course-index --skill line-memory
```
若你想全域安裝此規則（套用到所有專案的 Agent 中），只需加上 `-g` 參數：
```bash
npx skills add https://github.com/drgarbage/ag-course-index --skill line-memory -g
```
