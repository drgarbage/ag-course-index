...
- **CLI Presence & Logins**:
  ...
  - `gitleaks`: check with `command -v gitleaks`. This is required for Section 4's Secret Scan Gate — treat it as mandatory infrastructure, not optional. If missing, install it non-interactively (`brew install gitleaks` on macOS; on Windows, `winget install gitleaks.gitleaks` or download the release binary per `gitleaks`'s own install docs since it isn't in the course's winget catalog) and re-check `command -v gitleaks` before proceeding. Do not silently skip the scan gate because the binary is missing — halt and install first.
...
- **Vercel Preview Link Retrieval**: Do not parse the interactive `vercel list` table — it lists every historical deployment and can prompt interactively if the checkout isn't linked to the Vercel project. Prefer resolving the URL from GitHub's Deployments API, which Vercel's GitHub integration writes on every push regardless of whether a PR is open. `gh` already knows the current repo, so let it resolve owner/repo instead of hardcoding them:
  ```bash
  SHA=$(git rev-parse HEAD)
  # Vercel's webhook can take a few seconds after push, so poll instead of checking once.
  for i in $(seq 1 12); do
    ID=$(gh api "repos/{owner}/{repo}/deployments?sha=$SHA" --jq '.[0].id' 2>/dev/null)
    [ -n "$ID" ] && break
    sleep 5
  done
  gh api "repos/{owner}/{repo}/deployments/$ID/statuses" --jq '.[0].target_url'
  ```
  (`gh api repos/{owner}/{repo}/...` accepts the literal `{owner}/{repo}` placeholders — `gh` substitutes them from the current git remote automatically.) If the GitHub integration isn't installed, fall back to a non-interactive, JSON-based `vercel list` invocation filtered to the pushed commit (check `vercel list --help` for the current CLI's exact flag, since it varies by version) rather than the default table output.
  - **Polling Timeout**: 12 attempts × 5s (60s) covers a webhook delay but not a slow build. If the loop above exits with no `$ID`, do not silently give up or fabricate a URL — extend the poll (e.g. another 12 attempts at a longer interval) while telling the user a build is still in progress; if it still isn't found after ~5 minutes total, fall back to the `vercel list` JSON path, and if that also comes up empty, report plainly that the preview link isn't resolvable yet and give the user the Vercel dashboard project URL (`vercel project ls` / `vercel inspect`) as a manual fallback instead of leaving them with nothing.
