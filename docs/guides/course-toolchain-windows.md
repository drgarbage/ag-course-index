# Windows 課程開發環境一鍵安裝器指引

本指引說明如何使用 Windows 整合式一鍵安裝程式，快速建立、更新與檢查課程所需要的所有軟體工具鏈，並包含開發環境中文化設定。

## 快速安裝（建議）

適用於 Windows 10／11 系統。請依照以下步驟操作：

1. 按 **Windows 鍵 + R**，輸入 `powershell` 並按 **Enter** 開啟 PowerShell 視窗。
2. 複製以下完整指令並貼至 PowerShell 中，再按 **Enter** 執行：

```powershell
Set-ExecutionPolicy RemoteSigned -Scope CurrentUser; Invoke-RestMethod -Uri https://raw.githubusercontent.com/drgarbage/ag-course-index/main/scripts/install-course-toolchain-windows.ps1 | Invoke-Expression
```
...
### 1. 完整安裝 (全裝)
自動依序執行以下安裝與設定：
- **Git & GitHub CLI**：下載並安裝，並自動引導 Git 提交身分設定與 GitHub 帳號登入授權。
- **Node.js LTS**：安裝 Node.js 執行環境。
- **Cloudflare Tunnel (cloudflared)**：安裝內網穿透工具。
- **Docker Desktop**：檢查虛擬化支援與 WSL 2，並安裝 Docker Desktop。
...
### 2. 自訂安裝
如果您只想安裝特定工具或中文化，請選擇編號 `2`。您可以輸入編號進行多選（以逗號分隔，例如：`1,2,6`）：
1. Git 與 GitHub CLI
2. Node.js LTS
3. Cloudflare Tunnel
4. Docker Desktop
...
整合安裝器提供以下中文化支援：
1. **VS Code 開發環境中文化**：透過微軟市集語言包（`MS-CEINTL.vscode-language-pack-zh-hant`）自動切換為繁體中文。
